...
def dico_k_mers(K: int):
    """"""
    return {"".join(kmer): indice for indice, kmer in enumerate(itertools.product(string.ascii_lowercase+"!", repeat=K))}


def apprentissage_markov_k_mers(sequences: set[str], K: int) -> tuple[np.array]:
    """"""
    alphabet = dico_k_mers(K)
...
def logvraisemblance_markov_k_mers(sequence: str, A: np.array, PI: np.array, K: int, epsilon: float = 1e-10) -> float:
    """"""
    alphabet = dico_k_mers(K)
...
def generer_k_mers(A: np.array, PI: np.array, K: int, limite: int = 100) -> str:
    """"""
    alphabet = dico_k_mers(K)
    alphabet_inv = {indice: lettre for lettre, indice in alphabet.items()}
